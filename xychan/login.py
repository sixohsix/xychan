...
@get('/mod/login', name='login')
@view('login.tpl')
def login():
    return dict()


@post('/mod/log_me_in_please', name='login_submit')
@view('message.tpl')
def login_submit():
    ...
    if (not user) or (not user.verify_password(request.POST.get('password'))):
        ...
        return dict(message="You are now logged in", redirect=url('mod_home'))


@post('/mod/log_me_out_please', name='logout_submit')
@view('message.tpl')
def logout_submit():
